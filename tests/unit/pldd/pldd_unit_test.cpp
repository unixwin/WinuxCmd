...
TEST(pldd, pldd_lists_modules_for_process) {
  auto child = start_sleep_child();
  ASSERT_TRUE(child.valid());

  Pipeline p;
  p.add(L"pldd.exe", {std::to_wstring(child.pid())});

  auto r = p.run();

  EXPECT_EQ(r.exit_code, 0);
  ASSERT_CONTAINS(r.stdout_text, "powershell.exe");
  EXPECT_CONTAINS(r.stdout_text, "WindowsPowerShell");
}

TEST(pldd, pldd_name_mode_prints_basenames) {
  auto child = start_sleep_child();
  ASSERT_TRUE(child.valid());

  Pipeline p;
  p.add(L"pldd.exe", {L"--name", std::to_wstring(child.pid())});
...
TEST(pldd, pldd_rejects_invalid_pid) {
  Pipeline p;
  p.add(L"pldd.exe", {L"not-a-pid"});

  auto r = p.run();

  EXPECT_EQ(r.exit_code, 1);
  EXPECT_CONTAINS(r.stderr_text, "invalid process ID");
}
