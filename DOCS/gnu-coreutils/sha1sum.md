...
`sha1sum` 命令用于计算和检查 SHA-1（Secure Hash Algorithm 1）消息摘要。SHA-1 是一种加密哈希函数，产生 160 位（20 字节）的哈希值，通常用 40 位十六进制数字表示。
...
### `-c, --check`
从文件中读取 SHA-1 校验和，并验证每个文件的校验和是否匹配。文件格式为：每行包含一个 SHA-1 哈希值、一个空格、一个星号或空格（表示二进制或文本模式）、然后是文件名。
...
### `--tag`
输出 BSD 风格的校验和行，格式为：`SHA1 (文件名) = 哈希值`。
...
9. **严格模式验证**
   ```bash
   sha1sum -c --strict checksums.sha1
   ```
